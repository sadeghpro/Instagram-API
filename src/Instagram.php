...
class Instagram implements ExperimentsInterface
{
    ...
    {
        if (empty($apiPath)) {
            throw new \InvalidArgumentException('You must send api_path to get challenge');
        }

        return $this->_challenge($apiPath);
    }
     ...
     *
     * @param string $apiPath
     * @param string $method                            1 for email and 2 for phone number
     *
     *
     * @throws \InvalidArgumentException
     * @throws \InstagramAPI\Exception\InstagramException
    ...
    public function selectVerifyMethod(
        ...
        $method)
    {
        if (empty($apiPath) || !is_int($method)) {
            throw new \InvalidArgumentException('You must send api_path and method number to select verify method');
        }

//        if (is_int($method) && ($method>2 || $method<1)){
//            throw new \InvalidArgumentException('the correct value for method parameter is 1 (for selecting email) or 2 (for selecting phone number)');
//        }

        return $this->_selectVerifyMethod($apiPath, $method);
    }
    ...
    public function submitPhone(
        ...
        $phone)
    {
        if (empty($apiPath) || empty($phone)) {
            throw new \InvalidArgumentException('You must send api_path and phone number to submit phone');
        }

        return $this->_submitPhone($apiPath, $phone);
    }
    ...
    public function sendSecurityCode(
        ...
        $code)
    {
        if (empty($apiPath) || empty($code)) {
            throw new \InvalidArgumentException('You must send api_path and security code');
        }

        return $this->_sendSecurityCode($apiPath, $code);
    }
...
//                     }
     ...
     * @return array
     *
     * @see Instagram::challenge() The public login handler with a full description.
     */
        ...
        )
    {
        if (empty($apiPath)) {
            ...
            try {
                $response = $this->request($apiPath."?guid=".$this->uuid."&device_id=".$this->device_id)
                    ->setNeedsAuth(false)
                    ->getResponse(new Response\LoginResponse());
            } catch (\InstagramAPI\Exception\InstagramException $e) {
                throw $e;
     ...
     * @return array
     *
     * @see Instagram::selectVerifyMethod() The public login handler with a full description.
     */
    protected function _selectVerifyMethod(
        $apiPath,
        $method
    )
    {
...
//        }
     ...
     * @return array
     *
     * @see Instagram::submitPhone() The public login handler with a full description.
     */
    protected function _submitPhone(
        $apiPath,
        $phone
    )
    {
     ...
     * @return \InstagramAPI\Response\LoginResponse|null
     *
     * @see Instagram::sendSecurityCode() The public login handler with a full description.
     */
    protected function _sendSecurityCode(
        $apiPath,
        $code
    )
    {
